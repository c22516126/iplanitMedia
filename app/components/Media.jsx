...
// Media component's own DB operations
const dbOperations = {
  setupDB: async () => {
    return openDB('MediaDB', 1, {
      upgrade(db) {
        if (!db.objectStoreNames.contains('mediaImages')) {
          db.createObjectStore('mediaImages', { keyPath: 'id', autoIncrement: true });
        }
      }
    });
  },
  ...
  getAllImages: async () => {
    const db = await dbOperations.setupDB();
    return db.getAll('mediaImages');
  },

  addImage: async (image) => {
    const db = await dbOperations.setupDB();
    return db.add('mediaImages', {
      ...image,
      createdAt: new Date()
    });
  }
